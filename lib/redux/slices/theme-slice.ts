  ...
  reducers: {
    toggleTheme: (state) => {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem("lioncode_theme", state.mode);
      }
    },
    setTheme: (state, action: PayloadAction<ThemeMode>) => {
      state.mode = action.payload;
      if (typeof window !== "undefined") {
        localStorage.setItem("lioncode_theme", action.payload);
      }
    },
    syncThemeFromStorage: (state) => {
      if (typeof window !== "undefined") {
        const saved = localStorage.getItem("lioncode_theme") as ThemeMode | null;
